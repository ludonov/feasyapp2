...
export class ListPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public af: AngularFire, public alertCtrl: AlertController) {
    ...
    this.items_db.$ref.once("value", (snapshot: firebase.database.DataSnapshot) => {
      this.no_items = snapshot.numChildren() == 0;
    });
  }
  ...
  deleteList(): void {
    ...
    this.af.database.list("/unpublished_lists").remove(this.list.$key).then(res => {
