(function (factory) {
  ...
  Backendless._ajax_for_browser = function (config) {
    var cashingAllowedArr = ['cacheOnly', 'remoteDataOnly', 'fromCacheOrRemote', 'fromRemoteOrCache', 'fromCacheAndRemote']
      ...
      , sendRequest = function (config) {
        var xhr = new XMLHttpRequest(), contentType = config.data ? 'application/json' : 'application/x-www-form-urlencoded', response;
        ...
        var cacheHandler = function (response) {
          ...
          if (config.method == 'GET' && config.cacheActive) {
            ...
            '___class' in response && delete response['___class'];
            Backendless.LocalCache.set(config.urlBlueprint, response);
          }
        }
        ;
    ...
    config.cachePolicy = config.cachePolicy || {
      policy: 'ignoreCache'
    };
  ...
  function extendToLocalStorageCache(store) {
    ...
    store.flushExpired = function () {
      createBndlsStorage();
      var backendlessObj = store.deserialize(storage.getItem('Backendless')), obj;
      for (var prop in backendlessObj) {
        if (backendlessObj.hasOwnProperty(prop)) {
  ...
  DataStore.prototype = {
    ...
    find: function (dataQuery) {
      dataQuery = dataQuery || {};
      var props, whereClause, options, query = [], url = this.restUrl, responder = extractResponder(arguments), isAsync = responder != null, result;
  ...
  Cache.prototype = {
    put: function (key, value, timeToLive, async) {
      ...
      var responder = extractResponder([async])
        , isAsync = false;
      if (responder != null) {
        isAsync = true;
        responder = wrapAsync(responder);
  ...
  function enablePromises() {
    ...
    UserService.prototype.isValidLogin = function () {
      ...
      return Backendless.UserService.getCurrentUser().then(function (user) {
